...
@Component
@Slf4j
public class TaskHandleStopped implements ITaskHandle, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        TaskHandleFactory.registerTaskState(TaskStateEnum.STOPPED.getValue(), this);
    }

    /**
     * 开始启动中，但得到了停止状态
     * @param rtMonitor 监控信息
     * @param engineName 引擎
     * @return 执行结果
     */
    @Override
    public boolean startHandle(RtMonitor rtMonitor, String engineName) {
        ...
        // 2 更新监控表
        rtMonitor.setMonitorActual(TaskStateEnum.STOPPED.getValue());
    ...
    public boolean restartHandle(RtMonitor rtMonitor, String engineName) {
        ...
        if (RestartTypeEnum.Auto.getValue().equals(rtOpsAttachment.getRestartType())) {
            ...
            schedulerXxlJob.stopScheduler(rtMonitor.getXxlJobId());
            // 2 更新监控表中的相关信息
            rtMonitor.setMonitorActual(TaskStateEnum.FINISHED.getValue());
            monitorService.updateById(rtMonitor);
            // 3 更新,将重试表中的失败次数加1
