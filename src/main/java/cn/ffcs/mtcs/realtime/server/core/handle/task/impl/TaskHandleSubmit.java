...
@Component
@Slf4j
public class TaskHandleSubmit implements ITaskHandle, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        TaskHandleFactory.registerTaskState(TaskStateEnum.SUBMITTED.getValue(), this);
    }
    ...
    public boolean startHandle(RtMonitor rtMonitor, String engineName) {
        ...
        if (monitorCount >= rtMonitor.getMonitorNum()) {
            ...
            // 2 更新监控表
            rtMonitor.setMonitorActual(TaskStateEnum.SUBMITTED.getValue());
            ...
            String alarmMessage = rtOpsInfo.getOpsTeam() + "团队的"
                    + rtTaskInfo.getTaskName() + "任务，启动的状态为SUBMIT，需要介入排查！";
    ...
    public boolean restartHandle(RtMonitor rtMonitor, String engineName) {
        ...
        if (monitorCount >= rtMonitor.getMonitorNum()) {
            ...
            // 4 告警
            String alarmMessage = rtOpsInfo.getOpsTeam() + "团队的"
                    + rtTaskInfo.getTaskName() + "任务，重启的状态为SUBMIT，需要介入排查！";
            log.warn(alarmMessage);
            alarm.startException(alarmMessage);
        } else {
            rtMonitor.setMonitorActual(TaskStateEnum.SUBMITTED.getValue());
    ...
    public boolean runHandle(RtMonitor rtMonitor, String engineName) {
        ...
        // 2 告警
        String alarmMessage = rtOpsInfo.getOpsTeam() + "团队的"
                + rtTaskInfo.getTaskName() + "任务，状态为SUBMITTED，需要介入排查！";
        log.warn(alarmMessage);
        alarm.startException(alarmMessage);
