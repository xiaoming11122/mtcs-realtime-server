 ...
 * @Description .
 * @Author Nemo
 * @Date 2020/3/2/002 14:37
 * @Version 1.0
 */
@Component
@Slf4j
public class TaskHandleAccept implements ITaskHandle, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        TaskHandleFactory.registerTaskState(TaskStateEnum.ACCEPTED.getValue(), this);
    }
    ...
    public boolean startHandle(RtMonitor rtMonitor, String engineName) {
        ...
        if (monitorCount >= rtMonitor.getMonitorNum()) {
            ...
            // 2 更新监控表
            rtMonitor.setMonitorActual(TaskStateEnum.ACCEPTED.getValue());
            ...
            String alarmMessage = rtOpsInfo.getOpsTeam() + "团队的"
                    + rtTaskInfo.getTaskName() + "任务，启动的状态为ACCEPT，需要介入排查！";
            log.warn(alarmMessage);
            iAlarm.startException(alarmMessage);
        } else {
            rtMonitor.setMonitorActual(TaskStateEnum.ACCEPTED.getValue());
            monitorService.updateById(rtMonitor);
        }
    ...
    public boolean restartHandle(RtMonitor rtMonitor, String engineName) {
        ...
        if (monitorCount >= rtMonitor.getMonitorNum()) {
            ...
            // 4 告警
            String alarmMessage = rtOpsInfo.getOpsTeam() + "团队的"
                    + rtTaskInfo.getTaskName() + "任务，重启的状态为ACCEPT，需要介入排查！";
            log.warn(alarmMessage);
            iAlarm.startException(alarmMessage);
        } else {
            rtMonitor.setMonitorActual(TaskStateEnum.ACCEPTED.getValue());
    ...
    public boolean runHandle(RtMonitor rtMonitor, String engineName) {
        ...
        // 2 告警
        String alarmMessage = rtOpsInfo.getOpsTeam() + "团队的"
                + rtTaskInfo.getTaskName() + "任务，状态为ACCEPT，需要介入排查！";
        log.warn(alarmMessage);
        iAlarm.startException(alarmMessage);
