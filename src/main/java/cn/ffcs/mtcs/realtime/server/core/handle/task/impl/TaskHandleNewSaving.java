...
@Component
@Slf4j
public class TaskHandleNewSaving implements ITaskHandle, InitializingBean {
    @Autowired
    ...
    public void afterPropertiesSet() throws Exception {
        TaskHandleFactory.registerTaskState(TaskStateEnum.NEW_SAVING.getValue(), this);
    }
    ...
    public boolean startHandle(RtMonitor rtMonitor, String engineName) {
        ...
        if (monitorCount >= rtMonitor.getMonitorNum()) {
            ...
            // 2 更新监控表
            rtMonitor.setMonitorActual(TaskStateEnum.NEW_SAVING.getValue());
            ...
            String alarmMessage = rtOpsInfo.getOpsTeam() + "团队的"
                    + rtTaskInfo.getTaskName() + "任务，启动的状态为NEW_SAVING，需要介入排查！";
    ...
    public boolean restartHandle(RtMonitor rtMonitor, String engineName) {
        ...
        if (monitorCount >= rtMonitor.getMonitorNum()) {
            ...
            // 4 告警
            String alarmMessage = rtOpsInfo.getOpsTeam() + "团队的"
                    + rtTaskInfo.getTaskName() + "任务，重启的状态为NEW_SAVING，需要介入排查！";
            log.warn(alarmMessage);
            alarm.startException(alarmMessage);
        } else {
            rtMonitor.setMonitorActual(TaskStateEnum.NEW_SAVING.getValue());
    ...
    public boolean runHandle(RtMonitor rtMonitor, String engineName) {
        ...
        // 2 告警
        String alarmMessage = rtOpsInfo.getOpsTeam() + "团队的"
                + rtTaskInfo.getTaskName() + "任务，状态为NEW_SAVING，需要介入排查！";
        log.warn(alarmMessage);
        alarm.startException(alarmMessage);
